...
/* 
  ...
  The methods are:
   ...
   write()     - Sets the servo pulse width in microseconds or degrees. (0-200 as degrees, greater values are uS)
...
#define TRIM_DURATION      (SERVOS_PER_TIMER/2)    // compenstation ticks to trim adjust for digitalWrite delays 
...
uint8_t siezedTimers = 0;      // bitmap of initialized timers
...
#define SERVO(_timer,_channel)  (servos[(_timer*SERVOS_PER_TIMER) + (_channel)]) // macro to access servo class by timer and channel

#define SERVO_MIN(_servo_nbr) ((unsigned)(MIN_PULSE_WIDTH - servos[_servo_nbr].min * 4))  // minimum value in uS for this servo
#define SERVO_MAX(_servo_nbr) ((unsigned)(MAX_PULSE_WIDTH - servos[_servo_nbr].max * 4))  // maximum value in uS for this servo 
...
static inline void handle_interrupts(servoTimer_t timer, volatile uint16_t *TCNTn, volatile uint16_t* OCRnA)
{
  ...
  { 
    ...
    if( (unsigned)*TCNTn < (((unsigned int)REFRESH_INTERVAL * TICKS_PER_uS) + 4) )	// allow a few ticks to ensurethe next OCR1A not missed
      *OCRnA = (unsigned int)REFRESH_INTERVAL * TICKS_PER_uS;	
    else 
      *OCRnA = /*(unsigned)*/ *TCNTn + 4;  // at least REFRESH_INTERVAL has elapsed
...
{  
  for(uint8_t i=0; i < SERVOS_PER_TIMER; i++) { 
	servos[i + (timer*SERVOS_PER_TIMER)].ticks = DEFAULT_PULSE_WIDTH * TICKS_PER_uS;   // store default values	
  }
  if(timer == _timer1)
  {
...
static void initTimer(uint8_t servoIndex)
{
  // initialize the timer if it has not already been initialized 
  uint8_t timer = (uint8_t)SERVO_INDEX_TO_TIMER(servoIndex);
  if( (siezedTimers & (1<< timer)) == 0) // has the timer been siezed?
  {
    initISR((servoTimer_t)SERVO_INDEX_TO_TIMER(servoIndex));  
    siezedTimers |= (1<< timer);
  }
}
//#define DEBUG_SHOW
#ifdef DEBUG_SHOW
#include <HardwareSerial.h> 
void debugShow(int index)
{
  Serial.print("Servo dump: ");
...
void MegaServo::write(unsigned int value)
{	
...
{
  return  map( this->read_us(), SERVO_MIN(this->servoIndex), SERVO_MAX(this->servoIndex), 0, 180);     
}

int MegaServo::read_us()
{
  unsigned int pulsewidth;
  if( this->servoIndex > 0)
    pulsewidth = (servos[this->servoIndex].ticks - TRIM_DURATION) /  TICKS_PER_uS;
  else 
    pulsewidth  = 0;
